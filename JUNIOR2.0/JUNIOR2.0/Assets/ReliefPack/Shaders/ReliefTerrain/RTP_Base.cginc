...
//#define RTP_MAPPED_SHADOWS
	...
	#define UV_BLEND_SRC_0 (tex2Dlod(_SplatAtlasA, float4(uvSplat01M.xy, _MixMipActual.xx)).rgb)
	#define UV_BLEND_SRC_1 (tex2Dlod(_SplatAtlasA, float4(uvSplat01M.zw, _MixMipActual.yy)).rgb)
	#define UV_BLEND_SRC_2 (tex2Dlod(_SplatAtlasA, float4(uvSplat23M.xy, _MixMipActual.zz)).rgb)
	#define UV_BLEND_SRC_3 (tex2Dlod(_SplatAtlasA, float4(uvSplat23M.zw, _MixMipActual.ww)).rgb)
...
#define UV_BLEND_ROUTE_LAYER_4 UV_BLEND_SRC_2
#define UV_BLEND_ROUTE_LAYER_5 UV_BLEND_SRC_2
#define UV_BLEND_ROUTE_LAYER_6 UV_BLEND_SRC_2
#define UV_BLEND_ROUTE_LAYER_7 UV_BLEND_SRC_2
...
float4 _Spec0123;
float4 _Spec4567;
float4 _FarGlossCorrection0123;
...
//	return;
		...
		if (_uv_Relief_w<1) {
		 	...
		 	fixed4 col;
				...
				col.rgb = global_color_value.rgb*dot(splat_control, tex2D(_SSColorCombinedA, IN._uv_Relief.xy));
				o.Gloss=0;
		 	#else
			...
			o.Specular = 0.03+dot(_Spec0123, splat_control);
			o.Specular=lerp(o.Specular, 0.5, _uv_Relief_w);
			
			#if defined(SS_USE_BUMPMAPS)
				float3 n;
				float4 normals_combined;
				normals_combined = tex2D(_BumpMap01, IN._uv_Relief.xy).rgba*splat_control.rrgg;
				normals_combined+=tex2D(_BumpMap23, IN._uv_Relief.xy).rgba*splat_control.bbaa;
	...
	if (_uv_Relief_z>0) {
		...
		float shadow_atten=1;
			...
			float4 water_splat_control2=splat_control2;
					...
					TERRAIN_LayerReflection=dot(splat_control1, TERRAIN_LayerReflection0123)+dot(splat_control2, TERRAIN_LayerReflection4567)*_uv_Relief_z;
				#else
					TERRAIN_LayerReflection=dot(splat_control1, TERRAIN_LayerReflection0123)+dot(splat_control2, TERRAIN_LayerReflection4567);
				#endif
 			#endif
			#ifdef RTP_WETNESS
 				#if defined(RTP_SIMPLE_SHADING)
					TERRAIN_WetReflection=(dot(water_splat_control1, TERRAIN_WetReflection0123)+dot(water_splat_control2, TERRAIN_WetReflection4567))*_uv_Relief_z;
				#else
					TERRAIN_WetReflection=dot(water_splat_control1, TERRAIN_WetReflection0123)+dot(water_splat_control2, TERRAIN_WetReflection4567);
				#endif
				TERRAIN_LayerWetStrength=dot(splat_controlA_normalized, TERRAIN_LayerWetStrength0123)+dot(splat_controlB_normalized, TERRAIN_LayerWetStrength4567);
				TERRAIN_WetSpecularity=dot(water_splat_control1, TERRAIN_WetSpecularity0123)+dot(water_splat_control2, TERRAIN_WetSpecularity4567);
				TERRAIN_WaterGloss=dot(water_splat_control1, TERRAIN_WaterGloss0123)+dot(water_splat_control2, TERRAIN_WaterGloss4567);
				TERRAIN_WaterColor=half4( dot(splat_controlA_normalized, TERRAIN_WaterColorR0123)+dot(splat_controlB_normalized, TERRAIN_WaterColorR4567), dot(splat_controlA_normalized, TERRAIN_WaterColorG0123)+dot(splat_controlB_normalized, TERRAIN_WaterColorG4567), dot(splat_controlA_normalized, TERRAIN_WaterColorB0123)+dot(splat_controlB_normalized, TERRAIN_WaterColorB4567), dot(splat_controlA_normalized, TERRAIN_WaterColorA0123)+dot(splat_controlB_normalized, TERRAIN_WaterColorA4567) );
				
				float TERRAIN_WaterLevel=dot(water_splat_control1, TERRAIN_WaterLevel0123)+dot(water_splat_control2, TERRAIN_WaterLevel4567);
				float TERRAIN_WaterLevelSlopeDamp=dot(splat_controlA_normalized, TERRAIN_WaterLevelSlopeDamp0123)+dot(splat_controlB_normalized, TERRAIN_WaterLevelSlopeDamp4567);
...
//#endif		 	
			...
			float hbAO=(dot(splat_control1,RTP_AO_0123)+dot(splat_control2,RTP_AO_4567))*RTP_AOamp*2;
			...
			o.Specular = 0.03+dot(_Spec0123, splat_control1);
			#ifdef RTP_UV_BLEND
			#ifndef RTP_DISTANCE_ONLY_UV_BLEND
				float4 _MixMipActual=min(_MipActual.xxxx+log2(_MixScale0123), 6);
				...
				uvSplat01M=frac(IN._uv_Relief.xyxy*_MixScale0123.xxyy)*_multMix0123+_offMix0123;
				uvSplat01M.zw+=float2(0.5,0);
				uvSplat23M=frac(IN._uv_Relief.xyxy*_MixScale0123.zzww)*_multMix0123+_offMix0123;
...
//				uvSplat23M+=float4(0,0.5,0.5,0.5);			
				...
				#ifndef RTP_DISTANCE_ONLY_UV_BLEND			
					colBlend=lerp((dot(colBlend,0.33333)).xxx, colBlend, dot(splat_control1, _MixSaturation0123) + dot(splat_control2, _MixSaturation4567));
			...
			//
		        ...
		        float _WaterOpacity=(dot(splat_controlA_normalized, TERRAIN_WaterOpacity0123)+dot(splat_controlB_normalized, TERRAIN_WaterOpacity4567))*p; // gladsze przejscie (po nieskompresowanym splat_control)
		 	...
		 	//
				...
				for(int i=0; i<_TERRAIN_SHADOW_STEPS; i++) {
					...
					if (hit_flag) {
						...
						float scl=dh_prev / ((_h-h_prev) - EyeDirTan.z);
						rayPos.xyz-=EyeDirTan*(1 - scl); // back
						EyeDirTan=IN.lightDir.xyz*_TERRAIN_HeightMap_TexelSize.x*2*_TERRAIN_WAVELENGTH_SHADOWS;
						EyeDirTan.z/=max(0.001, _TERRAIN_ExtrudeHeight*COLOR_DAMP_VAL);
						float smooth_val=0;
...
//#endif		 			
			...
			float4 gloss=0;
				...
				if (triplanar_blend_flag) {
					...
					float heightblend_AO=1-saturate(1-dot(saturate(abs(splat_control1-0.5)*(2+RTP_AOsharpness)),0.25))*RTP_AOamp*dot(splat_control1,RTP_AO_0123);
					col*=heightblend_AO;
					gloss*=heightblend_AO;
				...
				gloss*=heightblend_AO;				
					...
					heightblend_AO=1-saturate(1-saturate((splat_controlA_coverage-0.5)*(2+RTP_AOsharpness)))*RTP_AOamp*dot(splat_control1,RTP_AO_0123)*0.5;
					col*=heightblend_AO;
					gloss*=heightblend_AO;				
					//float heightblend_AO2=saturate(1-saturate((splat_controlA_coverage-0.5)*20))*0.5;
					//col=lerp(col, global_color_value.rgb*heightblend_AO, heightblend_AO2);
				#endif
			...
			o.Specular = 0.03+dot(_Spec0123, splat_control1);
					...
					uvSplat01M=frac(IN._uv_Relief.xyxy*_MixScale0123.xxyy)*_multMix+_offMix;
					uvSplat01M.zw+=float2(0.5,0);
					uvSplat23M=frac(IN._uv_Relief.xyxy*_MixScale0123.zzww)*_multMix+_offMix;
					uvSplat23M+=float4(0,0.5,0.5,0.5);	
...
//#endif	 	
			...
			gloss*=heightblend_AO;
				...
				col*=heightblend_AO;
				gloss*=heightblend_AO;				
				//float heightblend_AO2=saturate(1-saturate((splat_controlB_coverage-0.5)*20))*0.5;
				//col=lerp(col, global_color_value.rgb*heightblend_AO, heightblend_AO2);
			#endif
...
//#endif		
			...
			float4 gloss=0;			
				...
				//
				#if !defined(_4LAYERS) || defined(USE_COLOR_ATLAS)
					float4 _MixMipActual=min(mip_selector.xxxx + rtp_mipoffset_color+MIPmult0123, 6);
			...
			#if defined(RTP_UV_BLEND) && !defined(RTP_TRIPLANAR)
				#if !defined(_4LAYERS) || defined(USE_COLOR_ATLAS)
					_MixMipActual=min(mip_selector.xxxx+rtp_mipoffset_color+log2(_MixScale0123), 6);
					float4 hi_mip_adjustMix=exp2(_MixMipActual)*_SplatAtlasA_TexelSize.x;
					_multMix-=hi_mip_adjustMix;
					_offMix+=0.5*hi_mip_adjustMix;
...
//				uvSplat23M=frac(IN._uv_Relief.xyxy*_MixScale4567.zzww)*_multMix+_offMix;
//				uvSplat23M+=float4(0,0.5,0.5,0.5);			
				_MixMipActual=min(_MixMipActual+log2(_MixScale0123), 6);
				float4 hi_mip_adjustMix=exp2(_MixMipActual)*_SplatAtlasA_TexelSize.x;
				_multMix-=hi_mip_adjustMix;
				_offMix+=0.5*hi_mip_adjustMix;
...
//		#endif
	...
	#ifdef _4LAYERS
	o.Gloss=lerp(o.Gloss, saturate(o.Gloss+dot(splat_control1, _FarGlossCorrection0123)), _uv_Relief_w);
	#else
	o.Gloss=lerp(o.Gloss, saturate(o.Gloss+dot(splat_control1, _FarGlossCorrection0123) + dot(splat_control2, _FarGlossCorrection4567)), _uv_Relief_w);
	#endif
