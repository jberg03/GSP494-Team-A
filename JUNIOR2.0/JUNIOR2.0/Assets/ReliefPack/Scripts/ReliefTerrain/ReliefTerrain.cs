...
public class ReliefTerrain : MonoBehaviour {
	...
	public void InitTerrainTileSizes() {
		...
		if (terrainComp) {
			...
			globalSettingsHolder.terrainTileSizeX=renderer.bounds.size.x;
			globalSettingsHolder.terrainTileSizeZ=renderer.bounds.size.z;
		}
	}
	...
	public void RefreshTextures(Material mat=null) { // mat used by geom blend to setup underlying mesh
		...
		if (terrainComp && !globalSettingsHolder.useTerrainMaterial && globalSettingsHolder.numTiles>1 && !mat) {
			...
			if (!terrainComp && !mat) {
				...
				if (renderer.sharedMaterial==null || renderer.sharedMaterial.name!="RTPMaterial") {
					renderer.sharedMaterial=new Material(Shader.Find("Relief Pack/Terrain2Geometry"));
					renderer.sharedMaterial.name="RTPMaterial";
				}
				globalSettingsHolder.use_mat=renderer.sharedMaterial; // local params to mesh material
			}
			#if !UNITY_3_5
				if (terrainComp) {
					if (globalSettingsHolder.useTerrainMaterial) {
...
//			globalSettingsHolder.SetShaderParam("_BumpMapGlobal", BumpGlobalCombined);
	...
	public void RefreshControlMaps(Material mat=null) {
		...
		Terrain terrainComp = (Terrain)GetComponent(typeof(Terrain));		
		if (!terrainComp && !mat) {
			globalSettingsHolder.use_mat=renderer.sharedMaterial; // local params to mesh material (sharedMaterial made above if needed)
		}
		#if !UNITY_3_5
			if (terrainComp && !mat) {
				if (globalSettingsHolder.useTerrainMaterial) {
